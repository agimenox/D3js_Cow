//3
const URL_DATASET_PEAK = "../dataset/created_data.csv";
...
//1
function peak_ppm_day(measureDay){
  if (measureDay == undefined) {
		return;
  }
  get_filter_data_day(URL_DATASET_PEAK,format_date(measureDay))
  ...
  // Load and process the data
    ...
    cow_data.sort((a, b) => a.sample_start_time - b.sample_start_time);
    // Set the domains for the x and y scales
    x.domain(d3.extent(cow_data, d => d.sample_start_time));
    ...
    svg.append("g")
      .style("font-size", "14px")
      .call(d3.axisLeft(y)
        .ticks((d3.max(cow_data, d => d.peak_with_animal_ppm) - 2000) / 100)
        .tickFormat(d => {
          if (isNaN(d)) return "";
    ...
    svg.append("text")
      ...
      .text("Total peak_with_animal_ppm");
    ...
    listeningRect.on("mousemove", function (event) {
      ...
      const yPos = y(d.peak_with_animal_ppm);
      ...
      tooltip
        ...
        .html(`<strong>Date:</strong> ${d.sample_start_time.toLocaleDateString()}<br><strong>peak_with_animal_ppm:</strong> ${d.peak_with_animal_ppm !== undefined ? (d.peak_with_animal_ppm / 1).toFixed(0) + ' PPM' : 'N/A'}`)
    });
